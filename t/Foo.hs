...
import Language.Haskell.TH.Lift.Internal (unsafeCodeCoerceCompat)
#endif
...
instance Lift (f (Fix f)) => Lift (Fix f) where
  lift = $(makeLift ''Fix)
#if MIN_VERSION_template_haskell(2,16,0)
  liftTyped = unsafeCodeCoerceCompat . lift
#endif
...
instance Lift (f (Fix2 f)) => Lift (Fix2 f) where
  liftTyped = unsafeCodeCoerceCompat . $(makeLift ''Fix2)
instance Lift a => Lift (Fam2 a Int Char) where
  liftTyped = unsafeCodeCoerceCompat . $(makeLift 'Fam2Prefix1)
instance (Eq a, Lift a) => Lift (Fam2 a Bool Bool) where
  liftTyped = unsafeCodeCoerceCompat . $(makeLift 'Fam2InstBool)
#endif
